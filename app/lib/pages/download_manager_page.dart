...
class DownloadManagerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final receiveSession = context.watch(
      serverProvider.select((state) => state?.session),
    );
    final currentSession = _currentSession(receiveSession);
    ...
    final onlineFingerprints = {
      for (final device
          in context.watch(nearbyDevicesProvider).allDevices.values)
    ...
    final remoteAttachments = chat.messages
        ...
        .where(
          (attachment) =>
              ...
              attachment.sourceFingerprint != myFingerprint,
        )
        .toList(growable: false);

    if (receiveSession?.status == SessionStatus.finished) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (context.mounted &&
            context.ref.read(serverProvider)?.session?.sessionId ==
                receiveSession?.sessionId) {
          context.ref.notifier(serverProvider).closeSession();
        }
      });
    }
      ...
      children: [
        ResponsiveListView(
          ...
          children: [
            ...
            Text(
              '${_activeCount(currentSession)} active - ${receiveHistory.length} completed - ${remoteAttachments.length} available',
              style: Theme.of(
                context,
            ...
            if (currentSession == null)
              ...
              _ActiveReceiveCard(session: currentSession),
            ...
            if (remoteAttachments.isEmpty)
              _EmptyState(
                ...
                title: 'No remote attachments',
              )
            else
              ...remoteAttachments.map(
                (attachment) => _RemoteAttachmentTile(
                  attachment: attachment,
                  online: onlineFingerprints.contains(
  ...
  ReceiveSessionState? _currentSession(ReceiveSessionState? session) {
    return session?.status == SessionStatus.finished ? null : session;
  }
...
class _ActiveReceiveCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedFiles = session.files.values
        .where((file) => file.status != FileStatus.skipped)
        .toList(growable: false);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            CustomProgressBar(
              progress: totalBytes == 0 ? 0 : currentBytes / totalBytes,
            ),
            const SizedBox(height: 8),
            Text(
            ...
            ...session.files.values.map((file) {
              final itemProgress = progress.getProgress(
                sessionId: session.sessionId,
                fileId: file.file.id,
              );
              return Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (file.status == FileStatus.sending)
                            ...
                            Text(
                              _fileStatusLabel(file.status),
                              style: TextStyle(
                                color: _fileStatusColor(context, file.status),
                              ),
                            ),
                        ],
                      ),
  ...
  String _sessionStatusLabel(SessionStatus status) {
    return switch (status) {
      ...
      SessionStatus.finishedWithErrors =>
        t.progressPage.total.title.finishedError,
      SessionStatus.canceledBySender =>
        t.progressPage.total.title.canceledSender,
      SessionStatus.canceledByReceiver =>
        t.progressPage.total.title.canceledReceiver,
      _ => status.name,
    };
  }
              ...
              children: [
                ...
                Text(
                  online
                      ...
                      : '${attachment.size.asReadableFileSize} - Source offline',
              ...
              PopupMenuButton<_HistoryOption>(
                ...
                itemBuilder: (context) {
                  return (entry.path != null
                          ? _historyOptionsAll
                          : _historyOptionsWithoutOpen)
                      .map((option) {
                        return PopupMenuItem<_HistoryOption>(
                          value: option,
